      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-20 grid lg:grid-cols-2 gap-12 items-center"> 
        ...
        <div> 
          ...
          <div className="flex flex-wrap gap-4"> 
            <button className="bg-gradient-main hover:bg-gradient-hover px-3 py-3 md:px-6 rounded-xl font-medium shadow-glow transition"> 
              🎬 Explore Movies 
            </button> 
            
            <button className="bg-white/10 hover:bg-white/20 px-3 py-3 md:px-6 rounded-xl font-medium backdrop-blur-md border border-white/10 transition"> 
              ▶ Watch Trailer 
            </button> 
          </div> 
        ...
        <div className="relative"> 
          ...
          <div className="flex justify-center mt-4 gap-2"> 
            {movies.map((_, index) => ( 
              <div 
                key={index} 
                className={`w-3 h-3 rounded-full ${ index === current ? "bg-accent" : "bg-white/30"}`} 
              /> 
            ))} 
          </div> 
        </div> 
...
{/* ✨ GLOW UNDER ARC */}
<div className="absolute bottom-0 w-full h-24 bg-gradient-to-b from-primary/20 to-transparent blur-2xl z-10"></div>
    </section> 
  ); 
}